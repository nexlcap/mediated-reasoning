...
`build_dynamic_module_generation_prompt()` asks the LLM to design a bespoke panel of analysts from scratch, naming each with a snake_case identifier (e.g. `enterprise_sales_motion`, `b2b_pricing_strategy`) and writing a tailored system prompt. A gap-check pass then evaluates coverage and can add up to 3 more specialists.
...
class FinalAnalysis(BaseModel):
    ...
    deactivated_disclaimer: str         # Disclaimer when modules are deactivated via weight=0
...
class SelectionMetadata(BaseModel):
    ...
    selected_modules: List[str] # Module names generated by the LLM panel-generation pass
    ...
    ad_hoc_modules: List[AdHocModule] # Additional specialists added by the gap-check pass
```
...
| `--list-modules` | List fixed fallback modules (used with `--no-auto-select`) and exit |
...
| Flag | Description |
|------|-------------|
| `--no-auto-select` | Disable dynamic panel generation; use fixed 3-module fallback (market, cost, risk) |
...
**Implementation:** A two-step LLM pre-pass runs by default: (1) `build_dynamic_module_generation_prompt()` asks the LLM to invent 3–7 specialist roles from scratch (no fixed pool consulted), each with a snake_case name and a tailored system prompt; (2) a gap-check pass evaluates coverage and can add up to 3 more specialists. All generated modules are instantiated via `create_dynamic_module()`. Disable with `--no-auto-select` to use the 3 fixed fallback modules (market, cost, risk). Falls back to defaults if the pre-pass fails.

### Why dynamic generation instead of fixed pool selection?

Fixed pool selection (`build_module_selection_prompt`) picked from 12 named modules. This meant "Should I expand to healthcare?" always chose from the same market/tech/legal/etc. labels regardless of the problem's actual analytical needs. A B2B pricing question needs `enterprise_sales_motion` and `churn_economics`, not `environmental` and `political`.

Dynamic generation invents the exact specialist roles the problem demands. The module names appear in the final report as the analytical lens applied, so `regulatory_approval_pathway` is more informative to the reader than a generic `legal` label. The infrastructure (`create_dynamic_module()`) was already in place for ad-hoc gap-fill modules; the only change was moving generation to the primary path instead of treating it as a fallback.
